...
class TestMSA(unittest.TestCase):
    ...
    def test_perform_msa(self, mock_open, mock_popen):
        ...
        msa.perform_msa(os.path.abspath('unittests/data/msa.fasta'),\
            os.path.abspath('outputs/output.aln'), 'muscle')
        mock_open.assert_called_once()
    
    @patch('subprocess.Popen')
    def test_perform_msa_exception(self, mock_popen):
        mock_popen.side_effect = Exception()
        msa.perform_msa('unittests/data/msa.fasta',\
            'outputs/output.aln', 'muscle')
    ...
    @patch('subprocess.Popen')
    @patch('builtins.open', new_callable=mock_open)
    def test_perform_msa_mafft(self, mock_open, mock_popen):
        mock_popen.return_value.communicate.return_value = (b'output', b'error')
        msa.perform_msa(os.path.abspath('unittests/data/msa.fasta'),\
            os.path.abspath('outputs/output.aln'), 'mafft')
        mock_open.assert_called_once()
    ...
    def test_perform_msa_mafft_output(self, mock_open, mock_popen):
        mock_popen.return_value.communicate.return_value = (b'output', b'error')
        msa.perform_msa(os.path.abspath('unittests/data/msa.fasta'),\
            os.path.abspath('unittests/output.aln'), 'mafft')
        mock_open.assert_called_once()

    def test_perform_msa_performance(self):
        msa.perform_msa(os.path.abspath('unittests/data/msa.fasta'),\
            os.path.abspath('unittests/msa_output.aln'), 'mafft')
        assert os.path.exists(os.path.abspath('unittests/msa_output.aln'))
